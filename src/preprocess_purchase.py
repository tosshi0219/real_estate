...
class PreprocessPurchase:
    def __init__(self,mode,data_version,encoder_version):
        ...
        if mode=='inference':
            ...
            self.encoder_path='../label/label_encoder_purhcase_{}.pickle'.format(self.encoder_version)
            self.save_path='../intermediate_data/preprocessed_purchase_for_learning_{}.pickle'.format(self.data_version)

        self.load_path='../intermediate_data/purchase_spec_{}.pickle'.format(self.data_version)

        self.spec=self.load_spec()
        self.spec=self.pre_price(self.spec)
        self.spec=self.pre_bid(self.spec)
        self.spec=self.pre_transportation(self.spec)
        self.spec=self.pre_land(self.spec)
        self.spec=self.pre_area(self.spec)
        self.spec=self.pre_structure(self.spec)
        self.spec=self.pre_age(self.spec)
        self.spec=self.pre_floor(self.spec)
        self.spec=self.pre_direction(self.spec)
        self.spec=self.pre_mcost(self.spec)
        self.spec=self.pre_rcost(self.spec)
        self.spec=self.encode_cat_to_label(self.spec)
        if mode =='learning':
    ...
    def load_spec(self):
        with open(self.load_path,'rb') as f:
            spec=pickle.load(f)
        spec=spec.drop(dropcol_purchase, axis=1)
        return spec
